...
static struct local_options
{
	...
	pthread_mutex_t mx_sort; 
	unsigned char selected_bssid[6]; 
						   ...
						   int cardnum)
{
	...
	if (ap_cur == NULL)
	{
		...
		ap_cur->channel_width = CHANNEL_22MHZ; 
...
static void dump_print(int ws_row, int ws_col, int if_num)
{
	...
	if (!lopt.singlechan) columns_ap -= 4; 
	if (lopt.show_uptime) columns_ap += 15; 
	nlines = 2;
...
static int getfrequencies(const char * optarg)
{
	...
	free(tmp_frequencies);
	free(optc);
	if (i == 1) return (lopt.own_frequencies[0]); 
	if (i == 0) return (-1); 
	return (0); 
}
